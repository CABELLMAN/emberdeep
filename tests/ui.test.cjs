...
function boot(initial) {
  ...
  for (const file of ['campaign.js', 'engine.js', 'town-ui.js', 'game.js']) vm.runInContext(fs.readFileSync(path.join(__dirname, '..', file), 'utf8'), context, { filename: file });
...
test('legacy combat loads without resetting, and victory/defeat CTAs return to town', () => {
  ...
  assert.equal(combat.state().version, 2); assert.equal(combat.nodes.get('town-btn').disabled, true);
