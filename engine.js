(function (root) {
  ...
  function createGame(seed='EMBER-'+Math.floor(Math.random()*99999)) { seed=String(seed).trim().slice(0,40)||'EMBER';const g={version:1,seed,floor:1,turn:0,phase:'explore',party:[{id:'warden',name:'Aldric',role:'Warden',hp:38,maxHp:38,attack:8,cooldown:0,acted:false},{id:'ranger',name:'Lyra',role:'Ranger',hp:28,maxHp:28,attack:7,cooldown:0,acted:false},{id:'arcanist',name:'Sera',role:'Arcanist',hp:26,maxHp:26,attack:6,cooldown:0,acted:false}],potions:4,gold:0,kills:0,level:1,xp:0,log:[],combat:null,map:generate(seed,1)};g.pos={...g.map.start};reveal(g);log(g,'Your torch catches. Somewhere below, the Emberheart still burns.');return g; }
  ...
  function setIntents(g) { const alive=g.party.map((h,i)=>h.hp>0?i:-1).filter(i=>i>=0); const rng=random(`${g.seed}/${g.floor}/${g.turn}/${g.combat.round}`);for(const e of g.combat.enemies){ const target=alive[Math.floor(rng()*alive.length)]; let type='strike',damage=e.attack;if(e.type==='boss'){const cycle=(g.combat.round-1)%3;if(cycle===1){type='charge';damage=0;}if(cycle===2){type='all';damage=9;}}else if(e.type==='hexer'&&g.combat.round%2===0){type='all';damage=4;}e.intent={type,damage,target};} }
  function beginCombat(g,entity) { const types=entity.type==='boss'?['boss','guard']:entity.kind==='rats'?['rat','rat']:entity.kind==='raiders'?['raider','rat']:entity.kind==='hexers'?['hexer','guard']:['guard','raider'];g.phase='combat';g.combat={entityId:entity.id,boss:entity.type==='boss',round:1,guard:false,enemies:types.map((t,i)=>enemy(t,i,g.floor))};g.party.forEach(h=>{h.acted=false;h.cooldown=0;});setIntents(g);log(g,entity.type==='boss'?'The Ash Warden rises. Bring the Emberheart home.':'An enemy blocks your path. Choose a hero and an action.','combat'); }
  function entityAt(g,x=g.pos.x,y=g.pos.y){return g.map.entities.find(e=>e.x===x&&e.y===y);}
  function move(g,dx,dy) { if(g.phase!=='explore'||Math.abs(dx)+Math.abs(dy)!==1)return false;const x=g.pos.x+dx,y=g.pos.y+dy;if(g.map.tiles[y]?.[x]!==1)return false;g.pos={x,y};g.turn++;reveal(g);const e=entityAt(g);if(e?.type==='enemy'||e?.type==='boss')beginCombat(g,e);else if(e?.type==='chest'){g.potions++;g.gold+=20+g.floor*5;g.map.entities=g.map.entities.filter(v=>v!==e);log(g,'A forgotten cache: +1 healing draught and a handful of gold.','loot');}else if(e?.type==='shrine')log(g,'A quiet shrine. Invoke it to heal and revive your party.');else if(e?.type==='stairs')log(g,'The stairwell leads deeper. Descend when you are ready.');return true; }
  function interact(g) { if(g.phase!=='explore')return false;const e=entityAt(g);if(e?.type==='shrine'){g.party.forEach(h=>{h.hp=Math.min(h.maxHp,h.hp+18);});g.map.entities=g.map.entities.filter(v=>v!==e);log(g,'The shrine rekindles every soul. All heroes recover 18 health.','heal');g.turn++;return true;}if(e?.type==='stairs'){g.floor++;g.level++;g.party.forEach(h=>{h.maxHp+=5;h.hp=Math.min(h.maxHp,h.hp+12);h.attack++;h.cooldown=0;});g.map=generate(g.seed,g.floor);g.pos={...g.map.start};g.turn++;reveal(g);log(g,`You enter ${FLOORS[g.floor-1]}. Party level ${g.level}: stronger, and restored by 12 health.`,'loot');return true;}return false; }
  ...
  function checkVictory(g) { if(g.combat.enemies.some(e=>e.hp>0))return false;const boss=g.combat.boss;g.kills+=g.combat.enemies.length;g.gold+=15*g.combat.enemies.length;g.xp+=g.combat.enemies.length*10;g.map.entities=g.map.entities.filter(e=>e.id!==g.combat.entityId);g.party.forEach(h=>{if(h.hp>0)h.hp=Math.min(h.maxHp,h.hp+4);h.acted=false;});g.phase=boss?'won':'explore';g.combat=null;log(g,boss?'The Warden falls. You reclaim the Emberheart, and the dark begins to lift.':'The chamber is yours. Living heroes recover 4 health.','loot');return true; }
  function enemyTurn(g) { for(const e of g.combat.enemies.filter(e=>e.hp>0)){const intent=e.intent;if(intent.type==='charge'){log(g,`${e.name} gathers embers. A devastating attack is coming.`,'combat');continue;}let targets=intent.type==='all'?g.party.filter(h=>h.hp>0):[g.party[intent.target]?.hp>0?g.party[intent.target]:g.party.find(h=>h.hp>0)].filter(Boolean);for(const h of targets){const damage=g.combat.guard?Math.ceil(intent.damage/2):intent.damage;h.hp=Math.max(0,h.hp-damage);log(g,`${e.name} hits ${h.name} for ${damage}${h.hp===0?' — fallen':''}.`,'danger');}if(g.party.every(h=>h.hp<=0)){g.phase='lost';log(g,'The torch goes dark. Your expedition has ended.','danger');return;}}g.combat.round++;g.combat.guard=false;g.party.forEach(h=>{h.acted=false;h.cooldown=Math.max(0,h.cooldown-1);});setIntents(g); }
  function act(g,heroIndex,action,targetIndex=0) { if(!Number.isInteger(heroIndex))return false;const h=g.party[heroIndex];if(!h||h.hp<=0||!['attack','special','potion','guard'].includes(action))return false;
    ...
    if(action==='special'){h.cooldown=2;if(h.id==='warden'){c.guard=true;log(g,'Aldric raises Bulwark. All incoming damage is halved this round.','heal');}else if(h.id==='ranger'){hurt(g,e,h.attack-2);if(e.hp>0)e.marked=2;log(g,'Lyra marks her quarry. The next two hits gain +3 damage.','combat');}else {log(g,'Sera unleashes an Ember Wave.','combat');c.enemies.filter(v=>v.hp>0).forEach(v=>hurt(g,v,h.attack+1));}}
  ...
  function validateSave(g) {
    try {
      ...
      if(g?.version!==1||typeof g.seed!=='string'||g.seed.length>40||!Number.isInteger(g.floor)||g.floor<1||g.floor>3||!['explore','combat','won','lost'].includes(g.phase)||!Number.isInteger(g.turn)||!num(g.turn))return false;
      const base=createGame(g.seed);
      if(!Array.isArray(g.party)||g.party.length!==3||g.party.some((h,i)=>h.id!==base.party[i].id||h.name!==base.party[i].name||h.role!==base.party[i].role||!num(h.hp,1000)||!num(h.maxHp,1000)||h.maxHp<1||h.hp>h.maxHp||!num(h.attack,1000)||!Number.isInteger(h.cooldown)||!num(h.cooldown,2)||typeof h.acted!=='boolean'))return false;
      const allDead=g.party.every(h=>h.hp===0);
      if((g.phase==='lost')!==allDead||g.phase==='won'&&g.floor!==3)return false;
      if(!Number.isInteger(g.potions)||!num(g.potions)||!num(g.gold)||!num(g.kills)||!Number.isInteger(g.level)||g.level!==g.floor||!num(g.xp))return false;
      const m=g.map;
      ...
      if(g.phase!=='won'&&m.entities.filter(e=>e.type===(g.floor<3?'stairs':'boss')).length!==1)return false;
      ...
      if(g.phase==='combat'||g.phase==='lost'){
        ...
        if(c.enemies.some(e=>!['rat','raider','guard','hexer','boss'].includes(e.type)||typeof e.name!=='string'||!num(e.hp,1000)||!num(e.maxHp,1000)||e.maxHp<1||e.hp>e.maxHp||!num(e.armor,1000)||!num(e.attack,1000)||!Number.isInteger(e.marked)||!num(e.marked,2)||!e.intent||!['strike','charge','all'].includes(e.intent.type)||!num(e.intent.damage,1000)||!Number.isInteger(e.intent.target)||e.intent.target<0||e.intent.target>2))return false;
        if(g.phase==='combat'&&(!c.enemies.some(e=>e.hp>0)||g.party.every(h=>h.hp<=0||h.acted)))return false;
      } else if(g.combat!==null)return false;
      return true;
    } catch {return false;}
  }
  const api={W,H,FLOORS,DIRS,key,random,generate,distances,createGame,move,interact,act,endRound,pathTo,entityAt,reveal,validateSave};
