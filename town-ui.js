...
(() => {
  ...
  const tabs = { tavern: 'The Lantern Inn', merchant: 'Treasure & supplies', weapons: 'Weapons', forge: 'Armor forge', training: 'Training grounds' };
  ...
  function render(g, selected) {
    const panel = tab === 'weapons' ? window.EmberWeaponUI.render(g, selected) : tab === 'merchant' ? merchant(g) : tab === 'forge' ? forge(g, selected) : tab === 'training' ? training(g) : tavern(g, selected);
