...
(() => {
  ...
  const tabs = { tavern: 'The Lantern Inn', formation: 'Formation', merchant: 'Treasure & supplies', weapons: 'Weapons', forge: 'Armor forge', training: 'Training grounds' };
  function tavern(g, selected) {
    const owned = new Set([...g.party, ...g.reserves].map(h => h.uid));
    return `<div class="town-panel-heading"><div><h3>A seat by the fire</h3><p>Your fellowship rests and revives here for free. ${g.party.length} of ${E.MAX_PARTY} adventurers travel; the others keep a room at the inn.</p></div><span class="town-badge">${owned.size} / ${E.STARTERS.length + Object.keys(E.RECRUITS).length} RECRUITED</span></div>
      <div class="town-notice">New recruits wait at the inn. <b>Add to party</b> fills an empty formation position; select an active hero to swap them instead. Arrange positions in <b>Formation</b>.</div>
      ${g.reserves.length ? `<h4 class="town-label">WAITING AT THE INN</h4><div class="town-card-grid">${g.reserves.map(h => `<article class="town-item"><span class="town-item-glyph">${glyphs[h.id]}</span><h4>${h.name}</h4><p>${h.role} · Level ${h.level}<br>${h.maxHp} HP · ${h.attack} ATK · ${h.armor} ARM</p><p class="item-detail">${E.equippedWeapon(h).name}<br>${armorSummary(h)}</p><button class="primary-btn" data-add-party="${h.uid}" ${disabled(g.party.length >= E.MAX_PARTY)}>${g.party.length >= E.MAX_PARTY ? 'Party full' : 'Add to party'}</button><button class="secondary-btn" data-swap="${h.uid}">Take ${esc(g.party[selected].name)}’s slot</button></article>`).join('')}</div>` : ''}
      <div class="formation-selected"><span><b>${esc(g.party[selected].name)}</b> · ${E.FORMATION_SLOTS[E.heroPosition(g, g.party[selected])].name}</span><button class="secondary-btn" data-bench="${selected}" ${disabled(g.party.length === 1)}>Rest at the inn</button></div>
  ...
  function forge(g, selected) {
    ...
    return `<div class="town-panel-heading"><div><h3>Arm ${esc(h.name)} for the descent</h3><p>Each location holds one piece in each layer. Choose every attribute independently, including weight and class suitability.</p></div><span class="town-badge">${h.role.toUpperCase()}</span></div>
      <div class="loadout"><span>⚔ ${E.equippedWeapon(h).name}</span><span>♜ ${armorSummary(h)}</span><strong>${h.attack} ATK · ${h.armor} ARM · ${h.maxHp} HP</strong></div>
      ...
      <button class="secondary-btn" data-town-tab="weapons">Browse weapons &amp; choose your grip</button>`;
  }
  function training(g) {
  ...
  function render(g, selected) {
    const panel = tab === 'formation' ? window.EmberFormationUI.town(g, selected) : tab === 'weapons' ? window.EmberWeaponUI.render(g, selected) : tab === 'merchant' ? merchant(g) : tab === 'forge' ? forge(g, selected) : tab === 'training' ? training(g) : tavern(g, selected);
